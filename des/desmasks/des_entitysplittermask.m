function varargout = des_entitysplittermask(block, action, subaction)
% Copyright 2004-2005 The MathWorks, Inc.
% $Revision: 1.1.6.4 $ $Date: 2007/11/07 18:26:20 $
...
case 'init',
    ...
    if(~des_validateinputs(block, MaskParamCheck))
        ...
        cbSplitEntityWhen(block, subaction);
...
%*********************************************************************
case 'default',
    des_def_entitysplittermask(block,mfilename);
    cbNumberOutputPorts(block,subaction);
    cbPortPrecedence(block, subaction);
    cbSplitEntityWhen(block,subaction);
    des_cbStats(block,'StatNumberArrived','#a',subaction);
...
% --- Field data
    ...
    case 'Round robin',
        ...
        [En{[idxInitialSeed]}]   = deal('off');
    otherwise,
	des_mask_error(block, 'DES_InvalidParameter', ['Illegal value for PortSelection  ' PortSelection ]);
    end
...
%*********************************************************************
% Function Name:    cbSplitEntityWhen
...
function cbSplitEntityWhen(block, applyStatus)
% --- Field data
    Vals = get_param(block, 'maskvalues');
    Vis = get_param(block, 'maskvisibilities');
    En = get_param(block,'MaskEnables');
    ...
    % --- Update the Mask Parameters if the below switch statement resulted
    %     in new values for the Visibility variables
    SplitEntityWhen = Vals{idxSplitEntityWhen};
    switch SplitEntityWhen,
    case 'All entity output ports are not blocked',
      	[Vis{[idxActionUponBlocking]}]   = deal('on');
        [En{[idxActionUponBlocking]}]   = deal('on');
        set_param(childName,'SubClassName','DES_SPLITTER_ALL');
    case 'Any entity output port is not blocked',
      	[Vis{[idxActionUponBlocking]}]   = deal('off');
        [En{[idxActionUponBlocking]}]   = deal('off');
        set_param(childName,'SubClassName','DES_SPLITTER_ANY');        
    otherwise,
	des_mask_error(block, 'DES_InvalidParameter', ['Illegal value for Split Entity When ' SplitEntityWhen ]);
    end
    % No dependent callabacks -- just set the visibilities and leave
    set_param(block, 'MaskVisibilities', Vis, 'MaskEnables', En);
return;

% --- end of des_entitysplittermask.m