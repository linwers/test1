function varargout = des_readtimermask(block, action, subaction)
% Copyright 2005 The MathWorks, Inc.
% $Revision: 1.1.8.4 $ $Date: 2007/11/07 18:26:39 $
...
case 'init',
    ...
    if(~des_validateinputs(block, MaskParamCheck))
        ...
        des_cbStats(block, 'StatTimerNumberDeparted', '#t', subaction);
        des_cbStats(block, 'StatAverageTimeElapsed' , 'w', subaction);
        cbStatTimeElapsed(block, subaction);
    end
...
%*********************************************************************
case 'default',
    des_def_readtimermask(block,mfilename);
    des_cbStats(block, 'StatNumberDeparted' ,'#d',subaction);
    des_cbStats(block, 'StatTimerNumberDeparted', '#t', subaction);
    des_cbStats(block, 'StatAverageTimeElapsed' , 'w', subaction);
    cbStatTimeElapsed(block, subaction);
...
case 'cbStatNumberDeparted'
    des_cbStats(block, 'StatNumberDeparted' ,'#d',subaction);
case 'cbStatTimerNumberDeparted',
   des_cbStats(block, 'StatTimerNumberDeparted', '#t', subaction);
case 'cbStatTimeElapsed',
    cbStatTimeElapsed(block, subaction);
case 'cbStatAverageTimeElapsed'
   des_cbStats(block, 'StatAverageTimeElapsed' , 'w', subaction);
...
%*********************************************************************
% Function Name:    cbStatStatTimeElapsed
...
%********************************************************************
function cbStatTimeElapsed(block, applyStatus)
    % --- Field data
    ...
    elapsedTimeOpt = Vals{idxStatTimeElapsed};
 
    switch elapsedTimeOpt,

    case 'On',
        des_enableport(block, applyStatus, 'out' , 'SL', 'et', 'yes');
    case 'Off',
        des_enableport(block, applyStatus, 'out' , 'SL', 'et', 'no');
    otherwise,
	des_mask_error(block, 'DES_InvalidParameter', ['Illegal value for elapsed time port option' ]);
    end
return;
...
% --- Subfunctions
% ----------------
function cbStatsTimer(block, applyStatus)
ParamNameArray = {'StatTimeElapsed', 'timeAvg'};
PortLabelArray = {'et'         , 'w'      };
