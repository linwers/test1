...
elseif iscell(x),
   ...
   des_error('DES_InvalidParam','Input must be either a string or a cell-array of strings.');
end
...
if ~iscell(c),
   des_error([mfilename ':InternalInvalidInputVars'],'Input must be a cell array of strings.');
end
...
for i=2:length(c),
   ...
   if ~ischar(str),
      des_error([mfilename ':InternalInvalidInputVars'],'Cell array must contain only strings.');
   end
   p=[p '|' str];
end
